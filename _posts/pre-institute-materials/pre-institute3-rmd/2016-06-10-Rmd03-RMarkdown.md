...
<a href="http://localhost:4000/tutorial-series/install-R-packages" target="_blank"> the setup materials</a> to get these installed. 
...
* **raster:** `install.packages("knitr")`
* **rgdal:** `install.packages("rmarkdown")`
...
HINT: the backticks can be found on your keyboard, on the same key as the tilde, 
not the apostrophe.
...
*  Delete all of the pre-populated text and code in your Rmd file. Be sure to NOT DELETE
the YAML header!
...
<pre><code>```{r load-dsm-raster }
   ...
   # import dsm
   teak_dsm <- raster("NEONdata/TEAK/lidar/Teak_lidarDSM.tif")
 ```</code></pre>
